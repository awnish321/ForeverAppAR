    ...
    OnBufferingUpdateListener, OnCompletionListener, OnErrorListener
{
        ...
        boolean playOnTextureImmediately, int seekPosition)
    {
        ...
        } else
        {
                ...
                (requestedType == MEDIA_TYPE.ON_TEXTURE_FULLSCREEN))) // or on texture with full screen
            {
                ...
                } else
                {
                    try
                    {
                      ...
                      /*  AssetFileDescriptor afd = mParentActivityRef.get().getAssets()
                            .openFd(filename);
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            SDCardRoot= new File("/storage/emulated/0/Android/data/foreverappar.com.foreverappar/files/Documents");
                        }else{
            ...
            {
                mPlayerHelperActivityIntent = new Intent(mParentActivityRef.get(),
    ...
    public boolean play(boolean fullScreen, int seekPosition)
    {
        if (fullScreen)
        {
            ...
            mParentActivityRef.get().startActivityForResult(mPlayerHelperActivityIntent, 1);
            return true;
        } else
        {
    ...
    {
        mParentActivityRef = new WeakReference<>(newActivity);
    }
    ...
    @SuppressLint("NewApi")
    boolean setupSurfaceTexture(int TextureID)
    {
